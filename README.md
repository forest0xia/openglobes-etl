...
|-------|--------|--------|---------|--------|---------|
| Aquatic | `merge_curated.py` | Curated + FishBase + OBIS | 200 species / 693 viewing spots | `final.json` + `hotspots.json` | CC-BY / CC-BY-NC |
...
```
curation/aquatic/
  selected.json             200 curated species with viewing spots
  hotspots.json             25 diving/viewing hotspot locations
data/raw/{globe}/           Raw downloads (parquet, CSV, GeoJSON, XML)
output/aquatic/
  final.json                Merged curated species (200) + ETL metadata
...
scripts/
  merge_curated.py          Merges curation + ETL metadata -> final.json
...
The aquatic globe uses a curated 200-species model instead of bulk tile-based output.

**Pipeline**: Hand-curated species list (`curation/aquatic/selected.json`) is merged with ETL-derived metadata (crosswalk, FishBase enrichment) via `scripts/merge_curated.py` to produce `output/aquatic/final.json`.

**200 curated species** across 3 tiers:
- **Star** (~50): Iconic species (blue whale, great white shark, manta ray)
- **Ecosystem** (~80): Ecologically important species (coral, kelp, sea urchin)
- **Surprise** (~70): Unusual or lesser-known species (frogfish, nudibranch, vampire squid)

**693 viewing spots**: Real-world diving/snorkeling/whale-watching locations linked to species and 25 hotspots.

**Sprites**: 179 photorealistic transparent PNG icons for globe markers. Each species resolves to a sprite via a 3-tier fallback chain: species sprite (179) -> group fallback (53) -> body-type fallback (10). Resolution happens at merge time.
